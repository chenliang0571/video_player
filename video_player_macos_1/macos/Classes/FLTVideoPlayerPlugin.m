...
- (instancetype)initWithURL:(NSURL*)url frameUpdater:(FLTFrameUpdater*)frameUpdater {
  ...
  NSLog(@"1 initWithURL frameUpdater: _playerItem %@", _playerItem);
...
- (instancetype)initWithPlayerItem:(AVPlayerItem*)item frameUpdater:(FLTFrameUpdater*)frameUpdater {
  ...
  _isPlaying = false;
  _disposed = false;
  NSLog(@"1 initWithPlayerItem %@", item);
...
- (void)observeValueForKeyPath:(NSString*)path
  ...
  NSLog(@"1 observeValueForKeyPath");
  if (context == timeRangeContext) {
    ...
    NSLog(@"1 observeValueForKeyPath item %@", item);
    switch (item.status) {
...
- (FLTTextureMessage*)onPlayerSetup:(FLTVideoPlayer*)player
  ...
  NSLog(@"1 flutter.io/videoPlayer/videoEvents%lld", textureId);
  FlutterEventChannel* eventChannel = [FlutterEventChannel
      eventChannelWithName:[NSString stringWithFormat:@"flutter.io/videoPlayer/videoEvents%lld",
                                                      textureId]
           binaryMessenger:_messenger];
  NSLog(@"1 eventChannel %@", eventChannel);
...
- (FLTTextureMessage*)create:(FLTCreateMessage*)input error:(FlutterError**)error {
  ...
  if (input.uri) {
    NSLog(@"1 input.uri %@", input.uri);
    player = [[FLTVideoPlayer alloc] initWithURL:[NSURL URLWithString:input.uri]
                                    frameUpdater:frameUpdater];
    NSLog(@"1 create player %@", player);
    FLTTextureMessage* result = [self onPlayerSetup:player frameUpdater:frameUpdater];
    NSLog(@"1 create result %@", result);
    return result;
  } else {
